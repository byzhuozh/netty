...
public class HashedWheelTimer implements Timer {
    ...
    public HashedWheelTimer(
            ...
            int ticksPerWheel,       // 一圈有几格  默认 512
            boolean leakDetection,      // 是否开启内存泄露检测
            long maxPendingTimeouts) {  // 最大延迟时间
    ...
    /**
     * 计算 ticksPerWheel 的值为不小于 ticksPerWheel 的最小2的n次方
     * @param ticksPerWheel
     * @return
     */
    ...
    public Set<Timeout> stop() {
        // worker 线程不能停止时间轮，也就是加入的定时任务，不能调用这个方法。
    ...
    /**
     * 时间轮的核心线程类。tick 的转动，过期任务的处理都是在这个线程中处理的
     */
    private final class Worker implements Runnable {
        ...
        @Override
        public void run() {
            // 初始化startTime. 所有任务的的deadline都是想对于这个时间点
            // Initialize the startTime.
            startTime = System.nanoTime();
            if (startTime == 0) {
                // 由于System.nanoTime()可能返回0，甚至负数。并且0是一个标示符，用来判断startTime是否被初始化，
            ...
            do {
                ...
                // 返回值就是System.nanoTime() - startTime, 也就是Timer启动后到这次 tick, 所过去的时间
        ...
        /**
         * 将 newTimeout() 方法中加入到待处理定时任务队列中的任务加入到指定的格子中
         */
        private void transferTimeoutsToBuckets() {
            // transfer only max. 100000 timeouts per tick to prevent a thread to stale the workerThread when it just
            // adds new timeouts in a loop.
            // 每次 tick 只处理10w 个任务，以免阻塞 work er线程
            for (int i = 0; i < 100000; i++) {
                ...
                //如果任务在timeouts队列里面放久了, 以至于已经过了执行时间, 这个时候就使用当前tick,
                // 也就是放到当前bucket, 此方法调用完后就会被执行.
     ...
     *
     * 用来存放 HashedWheelTimeout，结构类似于LinkedList
     */
    private static final class HashedWheelBucket {
        ...
        public void expireTimeouts(long deadline) {
            ...
            while (timeout != null) {
                ...
                if (timeout.remainingRounds <= 0) { // 时间轮为0，说明可以执行任务了
                    next = remove(timeout);
                    if (timeout.deadline <= deadline) {
                        timeout.expire();   //到期并执行任务
                    } else {

 ...
 *  2.writerIndex ，写索引。
 *  3.capacity ，当前容量。
 *  4.maxCapacity ，最大容量。当 writerIndex 写入超过 capacity 时，可自动扩容。每次扩容的大小，为 capacity 的 2 倍。
 *    当然，前提是不能超过 maxCapacity 大小
 *
 *  ByteBuf 通过 readerIndex 和 writerIndex 两个索引，解决 ByteBuffer 的读写模式的问题。
 *
 *  四个大小关系：readerIndex <= writerIndex <= capacity <= maxCapacity
 */

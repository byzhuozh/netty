package io.netty.example.byzhuozh.nio;
...
/**
 * @author zhuozh
 * @version $Id: NioClient.java, v 0.1 2018/10/25 0:30 zhuozh Exp $
 * @desc 采用芋艿 demo 学习   https://github.com/YunaiV
 */
public class NioClient {
    ...
    private void handleConnectableKey(SelectionKey key) throws IOException {
        ...
        clientSocketChannel.finishConnect();  // 确认通道连接已建立，方便后续IO操作（读写）不会因连接没建立而导致NotYetConnectedException异常。
        // log
    ...
    private void handleWritableKey(SelectionKey key) throws ClosedChannelException {
        ...
        for (String content : responseQueue) {
            // 打印数据
            System.out.println("handleWritableKey 写入数据：" + content);
            // 返回
    ...
    public synchronized void send(String content) throws ClosedChannelException {
        ...
        selector.wakeup(); // 唤醒已经阻塞的 selector
    }

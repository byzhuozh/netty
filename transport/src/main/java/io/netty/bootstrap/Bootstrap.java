...
public class Bootstrap extends AbstractBootstrap<Bootstrap, Channel> {
    ...
    private ChannelFuture doResolveAndConnect(final SocketAddress remoteAddress, final SocketAddress localAddress) {
        ...
        if (regFuture.isDone()) {
            ...
            // 同 ServerBootstrap 中的监听一样，在 io.netty.channel.AbstractChannel.AbstractUnsafe.register0() 方法中的
            // safeSetSuccess(promise) 方法被执行时进行的触发调用
    ...
    private ChannelFuture doResolveAndConnect0(final Channel channel, SocketAddress remoteAddress,
        ...
        try {
            ...
            // 解析远程地址 (也是异步执行，返回 Future 类型)
            final Future<SocketAddress> resolveFuture = resolver.resolve(remoteAddress);

            // 远程地址解析完成 (resolveFuture#result 包含两种状态： null 和 success)
            ...
            // 走到这步，说明远程解析还未完成（resolveFuture#result 是 UNCANCELLABLE 状态）
